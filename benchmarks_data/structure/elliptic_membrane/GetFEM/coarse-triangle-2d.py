...
mesh = gf.Mesh("load", "./mesh/coarse-triangle-2d.msh")
...
mfu.save("coarse-triangle-2d.mfu")

mim = gf.MeshIm(mesh, elements_degree * 2)
mim.save("coarse-triangle-2d.mim")
...
mfu.export_to_vtk(
    "coarse-triangle-2d.vtk", "ascii", mfd, sigmayy, "sigmayy", mfu, RHS, "RHS", U, "U"
)

m = pv.read("coarse-triangle-2d.vtk")
pl = pv.Plotter()
pl.add_mesh(m, show_edges=True, line_width=2, scalars="sigmayy")
pl.add_point_labels(
    m.points[0], [str(m["sigmayy"][0])], point_size=10, font_size=20, text_color="white"
)

for i in [19, 21, 23, 26, 28, 30, 31]:
    pl.add_point_labels(
        m.points[i],
...
pl.show(cpos="xy", screenshot="coarse-triangle-2d.png")
